...
function calcEstrutura(areaReal) {
  // ── Coeficientes paramétricos — residência unifamiliar (NBR 6118 / SINAPI / regras práticas) ──
  //
  // PILARES: 0,10 a 0,15/m² → usar 0,12 (1 pilar a cada ~8m²)
  //   Espaçamento econômico: 3–5m. Posicionamento: cantos + cruzamentos de paredes
  const pilares = Math.ceil(areaReal * 0.12);
  //
  // ESTACAS = PILARES (cada pilar tem sua estaca/sapata)
  const estacas = pilares;
  //
  // BALDRAME: perímetro externo estimado + paredes internas (~40% do perímetro)
  //   Perímetro externo ≈ 4 × √área (retângulo equivalente)
  //   Paredes internas ≈ 40% do perímetro externo
  const perimetroExt = +(Math.sqrt(areaReal) * 4 * 0.95).toFixed(2);
  const baldrame     = +(perimetroExt * 1.40).toFixed(2); // ext + internas
  //
  // VIGAS: baldrame (inferior) + respaldo (superior) ≈ 2× baldrame
  const vigas_ml = +(baldrame * 2).toFixed(1);
  //
  // VOLUME DE CONCRETO por elemento (taxas NBR 6118 / SINAPI)
  //   Sapata/estaca: Ø25cm, L=3m → 0,147m³/estaca
  //   Pilar: 14×40cm, pd=2,8m → 0,157m³/pilar
  //   Baldrame/viga: 12×25cm → 0,030m³/ml
  const vol_fund  = +(estacas * 0.147).toFixed(2);
  const vol_pilar = +(pilares * 0.157).toFixed(2);
  const vol_viga  = +(vigas_ml * 0.030).toFixed(2);
  //
  // AÇO por elemento (taxas médias de mercado)
  //   Pilares: 140 kg/m³ · Vigas: 90 kg/m³ · Sapatas: 80 kg/m³ · Laje: 75 kg/m³
  const aco_pilares  = +(vol_pilar * 140).toFixed(0);
  const aco_vigas    = +(vol_viga  *  90).toFixed(0);
  const aco_fund     = +(vol_fund  *  80).toFixed(0);
  //
  // LAJE, TELHADO, CONTRAPISO
  const laje       = +(areaReal * 0.90).toFixed(1);  // 90% da área (exclui varanda/garagem)
  const aco_laje   = +(laje * 0.12 * 75).toFixed(0); // vol laje (espessura ~12cm) × 75 kg/m³
  const telhado    = +(areaReal * 1.15).toFixed(1);  // área + beiral + inclinação
  const contrapiso = +(areaReal * 0.95).toFixed(1);  // interna + garagem
  //
  // ALVENARIA: (baldrame + paredes internas) × pé direito × 0,85 (desconta vãos)
  const alvenaria  = +(baldrame * 2.80 * 0.85).toFixed(1);
  const fachada    = +(perimetroExt * 2.80 * 0.45).toFixed(1); // ~45% ext é fachada pintada
  //
  // FÔRMA: pilar 0,80m²/ml altura + viga 0,35m²/ml + laje 1,05m²/m²
  const forma_pilar = +(pilares * 2.80 * 0.80).toFixed(1);
  const forma_viga  = +(vigas_ml * 0.35).toFixed(1);
  const forma_laje  = +(laje * 1.05).toFixed(1);
  //
  return {
    pilares, estacas, baldrame, vigas_ml,
    vol_fund, vol_pilar, vol_viga,
    aco_pilares, aco_vigas, aco_fund, aco_laje,
    forma_pilar, forma_viga, forma_laje,
    laje, telhado, contrapiso, alvenaria, fachada, perimetroExt,
  };
}
...
// ─── TIPOS DE AMBIENTE ────────────────────────────────────────────────────────
// molhada:true → recebe azulejo na parede (banheiro, coz., lavanderia, área serviço)
// altAz → altura do azulejo na parede (m) — 0 = sem azulejo
const TIPOS={
  sala:      {lb:"Sala/Jantar",     ic:"🛋️",piso:"porcelanato",molhada:false,rodape:true, coefEq:1.00,altAz:0},
  cozinha:   {lb:"Cozinha",         ic:"🍳",piso:"ceramica",   molhada:true, rodape:true, coefEq:1.00,altAz:1.50},
  quarto:    {lb:"Quarto",          ic:"🛏️",piso:"porcelanato",molhada:false,rodape:true, coefEq:1.00,altAz:0},
  suite:     {lb:"Suíte",           ic:"🛁",piso:"porcelanato",molhada:false,rodape:true, coefEq:1.00,altAz:0},
  banheiro:  {lb:"Banheiro",        ic:"🚿",piso:"ceramica",   molhada:true, rodape:false,coefEq:1.00,altAz:2.40},
  lavabo:    {lb:"Lavabo",          ic:"🪥",piso:"ceramica",   molhada:true, rodape:false,coefEq:1.00,altAz:1.50},
  lavanderia:{lb:"Lavanderia",      ic:"🫧",piso:"ceramica",   molhada:true, rodape:false,coefEq:1.00,altAz:1.50},
...
function calcGuiaCompras(tot,estr){
  const P=1.10;
  const A=tot.areaReal,Apin=tot.pinturaTotal,Apar=tot.azulejo,Aalv=estr.alvenaria;
  ...
  return[
    ...
    // revestimentos
    {g:"⬜ Revestimentos & Pisos",k:"Porcelanato retificado",v:+(tot.porcelanato*IDX.piso_m2.piso_fator).toFixed(1),un:"m²",obs:"⚠️ +10% perda inclusa — sala, quartos, circulação"},
    {g:"⬜ Revestimentos & Pisos",k:"Cerâmica piso PEI≥4",v:+(tot.ceramicaPiso*IDX.piso_m2.piso_fator).toFixed(1),un:"m²",obs:"⚠️ +10% perda inclusa — banheiro, garagem, serviço"},
    {g:"⬜ Revestimentos & Pisos",k:"Azulejo / Cerâmica parede",v:+(Apar*P).toFixed(1),un:"m²",obs:"⚠️ +10% perda inclusa — banheiro, cozinha, área serviço"},
...
function calcDim(amb){
  ...
  const piso=c*l,forro=piso,perim=2*(c+l);
  // Azulejo: apenas em ambientes molhados, altura conforme tipo
  const azulejo = (t.molhada && t.altAz>0) ? perim * t.altAz : 0;
  // Pintura paredes = área de paredes secas (acima do azulejo ou total se sem azulejo)
  // Pintura = (perímetro × pd) − azulejo + forro (teto sempre pintado)
  const areaParedesTotal = perim * pd;
  const pinturaPar = areaParedesTotal - azulejo;   // paredes secas
  const pinturaTotal = pinturaPar + forro;          // + teto
  const rodape = t.rodape ? perim : 0;
  const areaEq = piso * t.coefEq;
  return{piso,forro,perim,rodape,areaParedesTotal,azulejo,pinturaPar,pinturaTotal,areaEq};
}
function totalizar(ambs){
  return ambs.filter(a=>a.ativo).reduce((acc,a)=>{
    ...
    acc.perim     += d.perim;
    acc.rodape    += d.rodape;
    acc.azulejo   += d.azulejo;
    acc.pinturaPar+= d.pinturaPar;
    acc.pinturaTotal+=d.pinturaTotal;
    acc.areaReal  += d.piso;
    acc.areaEq    += d.areaEq;
    if(t.piso==="porcelanato") acc.porcelanato  += d.piso;
    else                        acc.ceramicaPiso += d.piso;
    return acc;
  },{piso:0,forro:0,perim:0,rodape:0,azulejo:0,pinturaPar:0,pinturaTotal:0,areaReal:0,areaEq:0,porcelanato:0,ceramicaPiso:0});
}
...
${ambs.filter(a=>a.ativo).map(amb=>{const d=calcDim(amb),t=TIPOS[amb.tipo]||TIPOS.quarto;return`<tr><td>${t.ic} ${amb.lb}</td><td style="color:#666">${amb.c}×${amb.l}×${amb.pd}m</td><td class="r">${fN(d.piso,2)}</td><td class="r">${d.rodape>0?fN(d.rodape,2)+"m":"—"}</td><td class="r">${d.azulejo>0?fN(d.azulejo,2):"—"}</td><td class="r">${fN(d.pinturaTotal,2)}</td><td class="r">${fN(d.areaEq,2)}</td></tr>`;}).join("")}
<tr class="total-row"><td colspan="2"><b>TOTAL</b></td><td class="r"><b>${fN(tot.piso,1)}</b></td><td class="r"><b>${fN(tot.rodape,1)}m</b></td><td class="r"><b>${fN(tot.azulejo,1)}</b></td><td class="r"><b>${fN(tot.pinturaTotal,1)}</b></td><td class="r"><b>${fN(tot.areaEq,1)}</b></td></tr>
</tbody></table>
${modoCalc==="cub"?`
...
<div class="footer"><span>Desenvolvido por <b>Rafael Rodrigues</b> · Eng. Civil · Especialista em Gerenciamento de Obras · CUB NBR 12721 · SINAPI · TCPO 13ª ed.</span><span>Gerado em ${data}</span></div>
  ...
  // ── Download direto como arquivo HTML (abre no browser e o usuário imprime com Ctrl+P)
  // Mais confiável que window.open que pode ser bloqueado por popup-blockers
  const blob=new Blob([html],{type:"text/html;charset=utf-8"});
  const url=URL.createObjectURL(blob);
  const a=document.createElement("a");
  a.href=url;
  const nomeArq=(obraName||"orcamento").replace(/[^a-zA-Z0-9]/g,"-").toLowerCase();
  a.download=`estimativa-${nomeArq}-${new Date().toISOString().slice(0,10)}.html`;
      ...
      <div style={{maxWidth:780,margin:"0 auto",padding:"14px 12px",opacity:anim?1:0,transform:anim?"translateY(0)":"translateY(8px)",transition:"all 0.18s"}}>
        ...
        {tab===2&&(<>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:6,marginBottom:14}}>
            ...
            <Card label="Pintura (par.+teto)" value={`${fN(tot.pinturaTotal,1)} m²`} cor={C.grn}/>
          </div>
          ...
          {/* Resumo estrutural calculado */}
          <div style={{background:C.s2,border:`1px solid ${C.org}30`,borderRadius:11,padding:"12px 14px",marginBottom:12}}>
            <div style={{fontSize:10,color:C.org,fontWeight:700,marginBottom:8,letterSpacing:1}}>RESUMO ESTRUTURAL CALCULADO</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6}}>
              {[
                {lb:"Pilares",v:estrAuto.pilares,un:"un",c:C.org,obs:"0,12/m² · espaç. 3–5m"},
                {lb:"Estacas/Sapatas",v:estrAuto.estacas,un:"un",c:C.org,obs:"= nº de pilares"},
                {lb:"Baldrame + int.",v:estrAuto.baldrame,un:"m",c:C.org,obs:"ext. + paredes int."},
                {lb:"Vigas (baldrame+respaldo)",v:estrAuto.vigas_ml,un:"m",c:"#fbbf24",obs:"2× baldrame"},
                {lb:"Vol. Concreto Fund.",v:fN(estrAuto.vol_fund,2),un:"m³",c:C.red,obs:"estacas 0,147 m³/un"},
                {lb:"Vol. Concreto Pilares",v:fN(estrAuto.vol_pilar,2),un:"m³",c:C.red,obs:"14×40cm · pd 2,8m"},
                {lb:"Vol. Concreto Vigas",v:fN(estrAuto.vol_viga,2),un:"m³",c:C.red,obs:"12×25cm · 0,030 m³/m"},
                {lb:"Aço Pilares",v:fN(estrAuto.aco_pilares,0),un:"kg",c:C.pur,obs:"140 kg/m³"},
                {lb:"Aço Vigas",v:fN(estrAuto.aco_vigas,0),un:"kg",c:C.pur,obs:"90 kg/m³"},
                {lb:"Aço Fundação",v:fN(estrAuto.aco_fund,0),un:"kg",c:C.pur,obs:"80 kg/m³"},
                {lb:"Aço Laje",v:fN(estrAuto.aco_laje,0),un:"kg",c:C.pur,obs:"75 kg/m³"},
                {lb:"Fôrma Pilares",v:fN(estrAuto.forma_pilar,1),un:"m²",c:C.cya,obs:"0,80 m²/m altura"},
                {lb:"Fôrma Vigas",v:fN(estrAuto.forma_viga,1),un:"m²",c:C.cya,obs:"0,35 m²/ml"},
                {lb:"Fôrma Laje",v:fN(estrAuto.forma_laje,1),un:"m²",c:C.cya,obs:"1,05 m²/m²"},
                {lb:"Área de Laje",v:fN(estrAuto.laje,1),un:"m²",c:C.blu,obs:"90% da área"},
                {lb:"Telhado (projeção)",v:fN(estrAuto.telhado,1),un:"m²",c:C.red,obs:"área + beiral"},
                {lb:"Contrapiso",v:fN(estrAuto.contrapiso,1),un:"m²",c:C.cya,obs:"95% da área"},
                {lb:"Alvenaria",v:fN(estrAuto.alvenaria,1),un:"m²",c:C.warn,obs:"baldrame × pd − vãos"},
              ].map(({lb,v,un,c,obs})=>(
                <div key={lb} style={{background:C.s3,borderRadius:8,padding:"7px 9px",border:`1px solid ${c}20`}}>
                  <div style={{fontSize:9,color:c,fontWeight:700,marginBottom:2}}>{lb}</div>
                  <div style={{fontSize:14,fontWeight:700,color:c}}>{v} <span style={{fontSize:9,fontWeight:400}}>{un}</span></div>
                  <div style={{fontSize:8,color:C.t3,marginTop:1}}>{obs}</div>
                </div>
              ))}
            </div>
          </div>

          {/* Ajuste manual (para quem tem projeto) */}
          <div style={{background:C.s2,border:`1px solid ${estrManual?C.org:C.bd}`,borderRadius:10,padding:"11px 13px",marginBottom:14}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:estrManual?10:0}}>
              <button onClick={()=>{setEstrManual(p=>!p);if(!estrManual)setEstrAdj({});}}
                style={{padding:"5px 12px",borderRadius:7,border:`1px solid ${estrManual?C.org:C.bd}`,background:estrManual?C.org+"20":"transparent",color:estrManual?C.org:C.t3,cursor:"pointer",fontSize:11,fontWeight:700}}>
                {estrManual?"🔓 Ajuste Manual Ativo":"🔒 Valores Automáticos — Clique para ajustar"}
              </button>
              {estrManual&&<span style={{fontSize:10,color:C.org}}>Digite os valores do seu projeto estrutural</span>}
            </div>
            {estrManual&&(
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:7}}>
                {[{k:"pilares",lb:"Pilares",un:"un",c:C.org},{k:"estacas",lb:"Estacas",un:"un",c:C.org},
                  {k:"baldrame",lb:"Baldrame",un:"m",c:C.org},{k:"vigas_ml",lb:"Vigas (ml)",un:"m",c:"#fbbf24"},
                  {k:"laje",lb:"Área Laje",un:"m²",c:C.blu},{k:"telhado",lb:"Telhado",un:"m²",c:C.red},
                  {k:"contrapiso",lb:"Contrapiso",un:"m²",c:C.cya},{k:"alvenaria",lb:"Alvenaria",un:"m²",c:C.warn},
                  {k:"fachada",lb:"Fachada",un:"m²",c:C.grn},
                ].map(({k,lb,un,c})=>(
                  <div key={k}>
                    <div style={{fontSize:9,color:c,marginBottom:3}}>{lb}</div>
                    <InN value={estrAdj[k]??estrAuto[k]} onChange={v=>setEstrAdj(p=>({...p,[k]:v}))} step={k==="baldrame"||k==="vigas_ml"?0.5:1} w={80} cor={c} suffix={un}/>
                  </div>
                ))}
              </div>
            )}
